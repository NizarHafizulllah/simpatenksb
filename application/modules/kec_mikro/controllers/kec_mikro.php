...
class kec_mikro extends admin_controller {
    
    var $controller;
    public function kec_mikro(){
        parent::__construct();
  ...
  //       exit();  
    ...
    function baru(){
       ...
       $this->set_subtitle("Tambah Izin TDI Industri Mikro");
        $this->set_title("Tambah Izin TDI Industri Mikro");
        $this->set_content($content);
        $this->cetak();
    }
...
if($this->form_validation->run() == TRUE ) { 
   ...
   function editdata(){
      ...
      //    exit();
        ...
        $this->set_subtitle("Edit Izin TDI Industri mikro ");
        $this->set_title("Edit Izin TDI Industri mikro ");
        $this->set_content($content);
        $this->cetak();
   ...
   function status(){
         ...
         $this->db->where('id',$id);
         $mikro = $this->db->get('mikro');
         $data_array = $mikro->row_array();
    ...
    $data['header'] = "Dokumen Persyaratan Pendirian Koperasi";
    ...
    $this->db->select('m.*, kec.kecamatan as nm_kecamatan,k.klasifikasi as klasif_perusahaan, kab.kota as kabupaten');

      $this->db->from("mikro m");
      $this->db->join('tiger_kecamatan kec','m.kecamatan=kec.id');
      $this->db->join('tiger_kota kab','m.kabupaten=kab.id');
      $this->db->join('klasifikasi_usaha k','k.id=m.klasif_perusahaan');
      // $this->db->where('id_birojasa', $id_birojasa);
    ...
    $data['controller'] = get_class($this);
    $data['header'] = "Dokumen Persyaratan Izin Pendirian Usaha";
    $data['query'] = $resx->row_array();
